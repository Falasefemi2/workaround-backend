...
func New(pool *pgxpool.Pool, cfg *config.Config) http.Handler {
	...
	userHandler.RegisterRoutes(r, authMiddleware, adminOrHR, rateLimiter.Limit)
	deptHandler.RegisterRoutes(r, authMiddleware, adminOrHR)
	designationHandler.RegisterRoutes(r, authMiddleware, adminOrHR)
